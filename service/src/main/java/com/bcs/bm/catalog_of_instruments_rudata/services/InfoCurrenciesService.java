...
@Transactional
@Service(InfoCurrenciesService.NAME)
public class InfoCurrenciesService extends AbstractRuDataService<InfoCurrencies> {

    public static final String NAME = "infocurrencies" + AbstractRuDataService.NAME;

    public InfoCurrenciesService(RepositoryFacade<Map, InfoCurrencies> repositoryFacade,
                                JsonProcessor jsonProcessor,
                                RestTemplate restTemplate,
                                ServiceProperties serviceProperties) {
        super(repositoryFacade, serviceProperties, jsonProcessor, restTemplate, InfoCurrencies.class);
    }
    ...
    public List<InfoCurrencies> upload(Token token) {
        try {
            ...
            final URI uri = new URI(serviceProperties.getRuData().getUrl() + "/Info/Currencies");
            ...
            final List<InfoCurrencies> result = repositoryFacade.upsert(this.clazz, jsonProcessor.apply((List<Map<String, Object>>) response.getBody()), LocalDateTime.now(ZoneOffset.UTC));
            return result;
        } finally {
