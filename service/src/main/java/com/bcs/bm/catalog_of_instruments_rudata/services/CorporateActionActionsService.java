...
@Transactional
@Service(CorporateActionActionsService.NAME)
public class CorporateActionActionsService extends AbstractRuDataService<CorporateActionActions> {

    public static final String NAME = "corporateactionactions" + AbstractRuDataService.NAME;

    @Autowired
    public CorporateActionActionsService(RepositoryFacade<Map, CorporateActionActions> repositoryFacade,
                                         JsonProcessor jsonProcessor,
                                         RestTemplate restTemplate,
                                         ServiceProperties serviceProperties) {
        super(repositoryFacade, serviceProperties, jsonProcessor, restTemplate, CorporateActionActions.class);
    }
    ...
    public List<CorporateActionActions> upload(Token token) {
        try {
            ...
            final URI uri = new URI(serviceProperties.getRuData().getUrl() + "/CorporateAction/Actions");
            ...
            final List<CorporateActionActions> result = repositoryFacade.upsert(clazz, jsonProcessor.apply((List<Map<String, Object>>) response.getBody()), LocalDateTime.now(ZoneOffset.UTC));
            return result;
        } finally {
